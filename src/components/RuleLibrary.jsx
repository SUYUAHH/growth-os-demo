...
export default function RuleLibrary({ candidates = [], currentTrend, onReview, onToast }) {
  const matched = candidates.find((candidate) => candidate.id === currentTrend?.id || candidate.postId === currentTrend?.id || candidate.title === currentTrend?.title)
  const [candidateId, setCandidateId] = useState(matched?.id || candidates[0]?.id || '')
