...
import GrowthAttributionDecision from '../components/GrowthAttributionDecision'
import PaidDistributionDecision from '../components/PaidDistributionDecision'
...
export default function Remix({ trend, activeConceptId, draft, savedConcepts, generated, generating, onGenerate, onSelectConcept, onDraftChange, onSave, onOpenDistribution, candidates, onReviewKnowledgeRule, onToast }) {
  ...
  return (
    <div className="page-content page-remix">
      ...
      <div className="source-strip"><div className="source-strip-icon"><ScanSearch size={17} /></div><div className="source-strip-copy"><span className="eyebrow">CURRENT INPUT · X OPPORTUNITY SIGNAL</span><strong>{trend.title}</strong><small>{trend.source} · {trend.views} impressions · {trend.audience}</small></div><StatusPill tone="positive">{trend.status}</StatusPill></div>
      ...
      <div className="remix-grid">
        ...
        <div className="remix-right">
          ...
          <div className="section-block editor-block"><div className="editor-head"><div><div className="eyebrow">04 · CONTENT ASSET</div><h3>{activeConcept.title}</h3></div><StatusPill tone={savedConcepts.includes(activeConcept.id) ? 'positive' : 'warning'}>{savedConcepts.includes(activeConcept.id) ? '待审核' : '草稿'}</StatusPill></div><div className="editor-meta"><span><FileText size={13} /> {trend.category}</span><span><Edit3 size={13} /> English · {trend.platform || 'X'}</span><span><span className="green-dot" /> AI generated</span></div><textarea value={draft} onChange={(event) => onDraftChange(event.target.value)} aria-label="二创内容草稿" /><div className="editor-footer"><span>{draft.length} chars · recommended 180-260</span><button className="secondary-button" onClick={onSave}><BookMarked size={15} />{savedConcepts.includes(activeConcept.id) ? '已保存' : '保存到资产库'}</button></div></div>
          <button className="primary-button full-button remix-next" onClick={onOpenDistribution}>进入分发优化 <ArrowRight size={15} /></button>
        </div>
      </div>
      <div className="growth-attribution-wide"><GrowthAttributionDecision trend={trend} /></div>
      <div className="paid-distribution-wide"><PaidDistributionDecision trend={trend} /></div>
    </div>
  )
}
