    ...
    <div className="page-content page-remix">
      ...
      <div className="remix-grid">
        <div className="remix-left">
          <div className="section-block"><SectionHeader eyebrow="01 · BREAKDOWN" title="一条内容，为什么有效？" detail="从表面选题拆到可迁移机制。" /><div className="breakdown-grid">{breakdownItems.map((item, index) => <div className="breakdown-item" key={item.label}><span className="breakdown-index">0{index + 1}</span><span className="breakdown-label">{item.label}</span><strong>{item.value}</strong><p>{item.detail}</p></div>)}</div><div className="principle-callout"><div className="callout-icon"><Lightbulb size={16} /></div><div><span className="eyebrow">REUSABLE PRINCIPLE</span><strong>把产品卖点翻译成用户愿意分享的生活判断。</strong><p>不复制“quiet luxury”的表面词汇，保留“从抽象感受切到具体日常”的叙事机制。</p></div><button className="icon-button" aria-label="保存原则" title="保存原则"><BookMarked size={16} /></button></div></div>
