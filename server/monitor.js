...
function tweetToTrend(tweet) {
  ...
  return {
    ...
    followers: `${(tweet.author?.followers || 0).toLocaleString()}`,
    likes: likes.toLocaleString(),
    likeRate: tweet.metrics?.impressions ? `${((likes / tweet.metrics.impressions) * 100).toFixed(1)}%` : '—',
    comments: (tweet.metrics?.replies || 0).toLocaleString(),
    publishedAt: tweet.createdAt ? new Date(tweet.createdAt).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '刚刚',
    recommendation: '先核对粉丝基数与互动质量，再决定是否收录；单条高互动不等于稳定热点。',
